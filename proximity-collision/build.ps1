# Builds the Proximity & Collision canvas app (.msapp) and solution zip, and optionally imports it.
# Usage:
#   .\build.ps1            -> builds dist\ProximityCollision.msapp and dist\ProximityCollision_unmanaged.zip
...
$appName = "wrk_proximitycollision_a4f2c"
$pkgName = "ProximityCollision"
...
Move-Item $msaprZip (Join-Path $stage "$pkgName.msapr") -Force

# 3. Stage YAML sources and pack the msapp
Copy-Item (Join-Path $root "canvas\Src") $stage -Recurse
$msapp = Join-Path $dist "$pkgName.msapp"
...
$zip = Join-Path $dist "${pkgName}_unmanaged.zip"
